...
import { deleteSubscriptionLimit, fetchSubscriptionLimitById, fetchSubscriptionLimit } from '../../../lib/api';
import SubscriptionLimitAdd from "../../forms/subscriptionLimit/Form";
...
export default function SubscriptionLimit() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await fetchSubscriptionLimit();
      setData(res.data);
    } catch (err) {
 ...
 const handleEdit = async (id: string) => {
     try {
       const res = await fetchSubscriptionLimitById(id);
       setEditingData(res);
   ...
   const confirmDelete = async () => {
     ...
     try {
       await deleteSubscriptionLimit(deleteTarget);
  ...
  return (
    <div className="p-6 flex flex-col items-center">
      <div className="bg-white  rounded-t-2xl shadow-lg  w-full max-w-7xl">
        <div className="mb-6 bg-blue-800 p-4 rounded-t-2xl text-white relative">
          <div className="flex items-center justify-between">
            ...
            <div className="w-1/3 text-center">
              <h1 className="text-xl font-bold">Subscription limits</h1>
            </div>
        ...
        <h1 className="text-2xl ms-2 font-bold">subscription  limits</h1>
        {isLoading ? (
            ...
            <table className="min-w-full text-sm text-left">
              <thead className="bg-gray-100 text-gray-700 uppercase text-xs">
                <tr>
                  ...
                  <th className="px-6 py-4">softwareLimit</th>
                  <th className="px-6 py-4">ownerLimit</th>
                  <th className="px-6 py-4">managerLimit</th>
                  <th className="px-6 py-4">employeeLimit</th>
              ...
              <tbody className="text-gray-700">
                {data.map((item, index) => (
                  <tr key={item.id || `${item.clientId}-${item.propertyId}-${index}`} className="border-t border-gray-200 hover:bg-gray-50">
                    <td className="px-6 py-3">{item.clientId}</td>
                    <td className="px-6 py-3">{item.propertyId}</td>
                    <td className="px-6 py-3">{item.softwareLimit}</td>
                    <td className="px-6 py-3">{item.ownerLimit}</td>
                    <td className="px-6 py-3">{item.managerLimit}</td>
                    <td className="px-6 py-3">{item.employeeLimit}</td>
            ...
            {data.map((item) => (
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-sm text-gray-700"><span className="font-medium">softwareLimit :</span> {item.softwareLimit}</p>
                  <p className="text-sm text-gray-700"><span className="font-medium">ownerLimit:</span> {item.ownerLimit}</p>
                  <p className="text-sm text-gray-700"><span className="font-medium">managerLimit:</span> {item.managerLimit}</p>
                </div>
